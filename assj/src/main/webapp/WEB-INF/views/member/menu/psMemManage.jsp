...
<title>개인회원 관리</title>
		...
		<!-- 본문 -->
		<div id="frame">
				<h3><b>개인회원 관리</b> <span class="glyphicon glyphicon-user"></span></h3>
				<hr>
				...
				<tr>
					...
					<tr>
					<td>${map.MEM_NO}</td>
					<td>${map.MEM_NAME}(${map.MEM_ID})</td>
					<td>
						<c:if test="${empty map.MEM_GENDER}">
					...
					<td><fmt:formatDate value="${map.MEM_BIRTH}" 
						pattern="yyyy-MM-dd" /></td>	
					<td>
						<c:if test="${empty map.MEM_ADDR}">
							<span style="color:gray">( 등록된 주소가 없습니다 )</span>
						</c:if>
						<c:if test="${!empty map.MEM_ADDR}">
							${map.MEM_ADDR} ${map.MEM_DETAILADDR}	
						</c:if>	
					</td>
					<td>
