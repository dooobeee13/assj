...
<script type="text/javascript">
	function send(form){		
		if(!form.pwd.value){
			...
			form.content.focus();
			return false;
		}else if(form.originPwd.value!=form.pwd.value){
			alert("게시글 비밀번호와 다릅니다.");
			form.pwd.focus();
			return false;
		}	
...
<!-- /사이드바 -->
		...
		<div class="col-md-9">
		<fieldset>
			<form role="form" class="form-inline" method="post" action="<c:url value='/member/menu/edit.do'/>" onsubmit="return send(this)">	
				...
				<hr>	
					<div class="form-group">
						...
						<input type="text" class="form-control" id="name" name="name" size="35" value="${vo.name}" readonly="readonly">
						&nbsp;&nbsp;&nbsp;&nbsp; 
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" name="title" id="title" size="73" value="${vo.title}">
					</div>	
					<div class="form-group">
						<label for="content">내용</label>&nbsp;&nbsp;  
						<textarea rows="12" class="form-control" cols="75" id="content" name="content">${vo.content}</textarea>
					</div>
				<br>
				<br>
				<div id="btnDiv">
					<button type="submit" class="btn btn-default btn-sm" id="okBtn">수정</button>&nbsp;
					<a href="<c:url value='/member/menu/qnaDetail.do?no=${vo.no}'/>"><button type="button" class="btn btn-default btn-sm" id="cancleBtn">취소</button></a><br>
				</div>
			</form>
		</fieldset>
		</div>
