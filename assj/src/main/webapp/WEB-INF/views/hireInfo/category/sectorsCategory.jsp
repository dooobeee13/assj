				...
				<div class="category thinScroll top">
					<ul>
						...
						<c:forEach var="vo" items="${topCateogryList}" varStatus="status">
							<li class="col-md-6 <c:if test='${status.first}'>selected</c:if>"><a href="#aTabs-${vo.secNo}"><label class="chklabel" data-top="${vo.secNo}">${vo.secName}</label></a></li>
						</c:forEach>
					</ul>
				</div>
			</div>
						...
						<ul id="aTabs-${top.secNo}">
							<c:forEach var="vo" items="${map[subKey]}">
								<li class="col-md-6">
									...
									<label class="chklabel" data-name="${top.secName}>${vo.secName}" data-top="${top.secNo}" for="sectors-${vo.secNo}">
